...
- (BOOL)isBlurSupported {
    _isBlurSupported = ([[FXBlurScheduler sharedInstance].platform rangeOfString:@"iPhone 4"].location == NSNotFound);
    return _isBlurSupported;
}
...
- (void)setUp
{
    ...
    if (!_blurEnabledSet) {
        _blurEnabled = self.isBlurSupported;
    }
...
{
    if (blurEnabled) {
        blurEnabled = self.isBlurSupported;
    }
